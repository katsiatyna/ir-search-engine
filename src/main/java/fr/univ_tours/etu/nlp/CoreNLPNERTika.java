 ...
 *
 */
public class CoreNLPNERTika implements NERecogniser {

    private static final Logger LOG = LoggerFactory.getLogger(CoreNLPNERTika.class);
    ...
    public Map<String, List<String>> recogniseAll(String text) {
        ...
        try {
            ...
            for (Object entry : entries) {
                String entityType = (String) firstField.get(entry);
                if (!names.containsKey(entityType)) {
                    names.put(entityType, new ArrayList<>());
                }
                Integer start = (Integer) secondField.get(entry);
                Integer end = (Integer) thirdField.get(entry);
                String name = text.substring(start, end);
    ...
    public static void main(String[] args) throws IOException {
        ...
        try (FileInputStream stream = new FileInputStream(args[0])) {
            ...
            CoreNLPNERTika ner = new CoreNLPNERTika();
